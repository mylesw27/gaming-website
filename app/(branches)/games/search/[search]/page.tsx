...
export default function Search({ params }: { params: { search: string } }) {
    ...
    useEffect(() => {
        async function searchGames() {
            try {
                const response = await fetch(`http://localhost:8000/api-v1/game/search/${params.search}`);
                if (!response.ok) {
            ...
            games.map((game, index) => (
              <a key={index} href={`/games/${game._id}`} className="block">
                <div className="p-4 text-slate-300 bg-gray-700 rounded-lg">
                  <img src={game.image} alt={game.title} className="object-cover w-full h-64 md:h-80 lg:h-96 rounded-lg" />
                  <p className="text-lg font-bold mb-2 md:text-xl">{game.title}</p>
